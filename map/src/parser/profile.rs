...
pub struct Entries<'src> {
    lines: Peekable<CrlfLines<'src>>,
}

impl<'src> Entries<'src> {
    pub fn new(source: &'src [u8]) -> Entries<'src> {
        Entries {
            lines: CrlfLines::new(source).peekable(),
        }
    }
}
...
impl<'src> Iterator for Entries<'src> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.lines.find_map(parse_entry_start).map(|id| {
            ...
            loop {
                ...
                if let Some((key, value)) = self.lines.next().and_then(parse_entry_value) {
